...
def glove_runner(filepath, outputdir=GLOVE_WEIGHTS_PATH, mode="test", load_embedding=True):
    ...
    def preprocess_split(split, mode):
    ...
    if mode == 'train':
        ...
        train = preprocess_split(train, mode)
        eval = preprocess_split(eval, mode)
        print("Preparing embeddings")
        embedding_model = prepare_embedding_model(train, load_embedding)
    elif mode == 'test':
        test = preprocess_split(data, mode)
        print("Preparing embeddings")
    ...
    if mode == 'train':  
        ...
        compute_glove_predictions(model, X_test, X_test_ids, X_test_doc_tokens, outputdir)
